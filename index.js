...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = arr.values();
  return Array.from(positiveArrayValues).filter((value) => value > 0);
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum1 = arr.slice(-n);
  sum1.forEach((element) => console.log(element));
  const sum = sum1.reduceRight((x, y) => {
    return x + y;
  }, 0);
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((word, wordNext) => {
    if (word.length > wordNext.length) {
      return word;
    } else {
      return wordNext;
    }
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((sum, product) => {
    if (typeof product === "object" && product.hasOwnProperty("price")) {
